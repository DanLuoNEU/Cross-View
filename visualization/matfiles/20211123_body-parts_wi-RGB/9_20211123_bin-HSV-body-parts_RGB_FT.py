# This file is used to Draw RGB gif with Binary Codes HSV binarization aside
# 11/23/2021, Dan
...
def weightPoles(path_exp,  
    ...
    figs_bin, axs_bin = plt.subplots(len(part_body),1,gridspec_kw={'height_ratios': [1,1,1,1,2,2]},figsize=(4,8))
    ...
    figs_dict_color.savefig(os.path.join(path_exp,f"Dict_same-color-Bin_Ori-Clean.png"),dpi=400)

    # Plot Binarization with H-theta,S-rho,V-binarization
    for name_bin in list_name_bin:
        if 'action04' not in name_bin:
            continue
        path_bin = os.path.join(path_exp, name_bin)
        mat_bin = scipy.io.loadmat(path_bin)

        bin_gumbel = np.squeeze(mat_bin['bi_action']) 
        ...
        if draw_bin:
            for id_plot_bin in range(RGB_Gumbel.shape[0]):
                ...
                if id_plot_bin<12:
                    ...
                    imageio.mimsave(os.path.join(path_exp, f"RGB-{name_bin.split('.')[0].replace('coeff_bi_img_','')}_{id_plot_bin}.gif"),frames,fps=30)
                # Binary Code
                figs_bin, axs_bin = plt.subplots(len(part_body),1,gridspec_kw={'height_ratios': [1,1,1,1,2,2]},figsize=(4,8))
                figs_bin.suptitle(name_bin.split('.')[0].replace('coeff_bi_img_',''), fontsize=12)
                for i_body in range(len(part_body)):
                    # x+y
                    axs_bin[i_body].set_title(part_body[i_body],fontsize=8,loc='left')
                    im_bin=axs_bin[i_body].imshow(RGB_Gumbel[id_plot_bin,:,index_body[i_body]], interpolation='none',aspect="auto")
                    # x | y
                    # axs_bin[i_body,0].set_title(part_body[i_body]+'_X',fontsize=8,loc='left')
                    # axs_bin[i_body,1].set_title(part_body[i_body]+'_Y',fontsize=8,loc='left')
                    # im_bin=axs_bin[i_body,0].imshow(RGB_Gumbel[id_plot_bin,:,index_body[i_body][::2]], interpolation='none',aspect="auto")
                    # im_bin=axs_bin[i_body,1].imshow(RGB_Gumbel[id_plot_bin,:,index_body[i_body][1::2]], interpolation='none',aspect="auto")

                figs_bin.tight_layout(pad=0)

                # figs_bin.savefig(os.path.join(path_exp, f"B-code-{name_bin.split('.')[0].replace('coeff_bi_img_','')}_{id_plot_bin}_xy.png"), dpi=400)
...
def main():
    try:
        ...
        path_exp="/home/dan/ws/2021-CrossView/matfiles/1123_body-parts_wi-RGB"
