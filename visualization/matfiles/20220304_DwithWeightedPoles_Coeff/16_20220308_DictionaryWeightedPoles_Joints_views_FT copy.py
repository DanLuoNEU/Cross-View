# 03/08/2022, Dan
# This file is used to draw Dictionary with weighted poles depending on 
# DYAN Coefficients for several joints for setup3, v3 as test
...
dir_default='/home/dan/ws/2021-CrossView/matfiles/20220304_DwithWeightedPoles_Coeff/20211129/setup3'
...
def weightPoles(path_exp, num_used_top=10, dim_data_draw=0,
    ...
    dir_save = os.path.join(path_exp,'20220308_0_DictWeightedPoles_DYANcoeff')
    ...
    for name_bin in list_name_C_B:
        ...
        list_mat_bin.append((view_exp, scipy.io.loadmat(path_bin)))
    ...
    for id_act in range(len(list_mat_bin[0][1]['coeff_action'][0])):
        for name_joint, index_joint in dict_arm.items():

            for name_view,mat_bin in list_mat_bin:
                if name_view=='v1':    id_subplot=0
                elif name_view=='v2':  id_subplot=1
                elif name_view=='v3':  id_subplot=2
                # if id_act==0: continue
                bi_action_act=mat_bin['coeff_action'][0,id_act].squeeze()
                ...
                for legend_dim_data, id_dim_data in dict_dim_data.items():
                    ...
                    size_point_rho = weight_avg_poles_rho[ind_poles_rho,id_dim_data]*1000//2
                    ...
                    if id_dim_data==0:
                        axs_dict_joint[id_dim_data, id_subplot].set_title(f'{name_view}-{legend_dim_data}-[{min_avg_weight:.03f},{max_avg_weight:.03f}]', fontsize=10)
                    else:
                        axs_dict_joint[id_dim_data, id_subplot].set_title(f'{name_view}-{legend_dim_data}-[{min_avg_weight:.03f},{max_avg_weight:.03f}]', fontsize=10, y=-0.30)
                    axs_dict_joint[id_dim_data, id_subplot].tick_params(axis='x', labelsize=5)
                    axs_dict_joint[id_dim_data, id_subplot].tick_params(axis='y', labelsize=5)
                    for id_size, id_pole in enumerate(ind_poles_rho):
                        r_t = Drr_temp[id_pole]
                        theta_t = Dtheta_temp[id_pole]
                        
                        axs_dict_joint[id_dim_data,id_subplot].scatter( theta_t, r_t, s=size_point_rho[id_size], c=color_views[name_view], alpha=0.8, edgecolors='none')

            figs_dict_joint.suptitle(f"Act{id_act+1}-{name_joint}",fontsize=12)
            figs_dict_joint.savefig(os.path.join(dir_save, f'Act{id_act+1}-{name_joint}_{num_seqs*num_clips}clips_D-coeff.png'), dpi=400)
            for axs_temp in axs_dict_joint.flat: axs_temp.clear() 
    pass
