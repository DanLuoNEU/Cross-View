...
def weightPoles(path_exp,  
    ...
    figs_dict_color.savefig(os.path.join(path_exp,f"Dict_same-color-Bin_Ori.png"),dpi=400)
    ...
    for name_bin in list_name_bin:
        ...
        bin_gumbel = np.squeeze(mat_bin['bi_'+name_bin.split('.')[0].split('_')[-1]]) # 1x161x50
        ...
        for dim_HSV_0 in range(HSV_gumbel.shape[0]):
            for dim_HSV_1 in range(HSV_gumbel.shape[2]):
                ...
                HSV_gumbel[dim_HSV_0, 1:num_poles_b//2+1, dim_HSV_1, 0],HSV_gumbel[dim_HSV_0, num_poles_b//2+1:, dim_HSV_1, 0]=np.squeeze(Dtheta_Dict/(2*np.pi)), np.squeeze((2*np.pi-Dtheta_Dict)/(2*np.pi)) # H
        ...
        for id_plot_bin in range(RGB_Gumbel.shape[0]):
            axs_bin.set_title(name_bin.split('.')[0].replace('coeff_bi_Y_',''), fontsize=12)
            im_bin=axs_bin.imshow(RGB_Gumbel[id_plot_bin,].transpose(1,0,2), interpolation='none')
            # figs_bin.colorbar(im_bin,ax=axs_bin)
            if draw_bin:
                # figs_bin.savefig(os.path.join(path_exp, f"B-code-{name_bin.split('.')[0].replace('coeff_bi_Y_','')}_{id_plot_bin}_no-repeat-poles.png"), dpi=400)
                figs_bin.savefig(os.path.join(path_exp, f"B-code-{name_bin.split('.')[0].replace('coeff_bi_Y_','')}_{id_plot_bin}_ori.png"), dpi=400)
...
def main():
    try:
        ...
        path_exp="/home/dan/ws/2021-CrossView/matfiles/1119"
