# 03/02/2022, Dan
# This file is used to draw all Coefficients and Binary Codes HSV binarization for setup3, v3 for test
# Because we already have binary visualizations, just change V in HSV from binary value to coefficient absolute value
# Then get the coefficients visualizations
...
dir_default='/home/dan/ws/2021-CrossView/matfiles/20220302_DwithWeightedPoles/20211129/setup3'
...
def weightPoles(path_exp,  
    ...
    for name_bin in list_name_C_B:
        ...
        for id_act in range(len(mat_bin['coeff_action'][0])):
            if id_act==0: continue
            bi_action_act=mat_bin['coeff_action'][0,id_act]
            for id_vid in range(bi_action_act.shape[0]):
                bin_gumbel = np.squeeze(bi_action_act[id_vid]) 
                ...
                HSV_gumbel = np.repeat(np.abs(HSV_gumbel[:, :, :,]), 3, axis=3) # V
                ...
                if draw_bin:
                    for id_plot_bin in range(RGB_Gumbel.shape[0]):
                        ...
                        if not os.path.exists(os.path.join(dir_save,str(id_act),str(id_vid))):
                            os.makedirs(os.path.join(dir_save,str(id_act),str(id_vid)))
                        # # Binary Code
                        # path_output=os.path.join(dir_save,str(id_act),str(id_vid), f"B-code-{name_bin.split('.')[0].replace('coeff_bi_','')}_{id_plot_bin}_xy.png")
                        # Coefficient Values
                        path_output=os.path.join(dir_save,str(id_act),str(id_vid), f"Coeff-{name_bin.split('.')[0].replace('coeff_bi_','')}_{id_plot_bin}_xy.png")
                        # figs_B.savefig(path_output, dpi=400)
