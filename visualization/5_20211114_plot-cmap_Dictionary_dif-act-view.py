...
def weightPoles(path_exp, theta_max_pi=False, num_used_top=30, dim_data_draw=0, 
                weight_complex=False, draw_weight=False, draw_traj=False, draw_cmap=False):
    ...
    figs_p, axs_p = plt.subplots(num_subplots//3 if num_subplots%3==0 else num_subplots//3+1, 3, subplot_kw=dict(projection='polar'),figsize=(18,18))
    figs, axs = plt.subplots(num_subplots//3 if num_subplots%3==0 else num_subplots//3+1, 3,figsize=(18,18))
    figs_cmap, axs_cmap = plt.subplots( 12, num_subplots,figsize=(30,18))
    ...
    Drr_Dict = mat_Dict_DYAN['Drr']
    Dtheta_Dict = mat_Dict_DYAN['Dtheta']
    for id_subplot, name_mat in enumerate(list_name_Coeff):
        ...
        axs_cmap[0,id_subplot%4].set_title(name_mat.split('.')[0], fontsize=12)
        for row_cmap in range(weight_poles_cmap.shape[0]):
            axs_cmap[row_cmap,id_subplot%4].pcolormesh(weight_poles_cmap[row_cmap,].transpose())
        

        
        ind_C = np.argsort(weight_poles[:, dim_data_draw])[-num_used_top:]
        # Plot Circle
        axs_p[id_subplot//3,id_subplot%3].set_title(name_mat.split('.')[0], fontsize=12)
        axs_p[id_subplot//3,id_subplot%3].scatter(0,1,c='black')
        ...
        for id_size,id_C in enumerate(ind_C):
            if not theta_max_pi:
                ...
                if id_quadrant == 0:
                    ...
                    axs_p[id_subplot//3,id_subplot%3].scatter( theta_t, r_t, s=size_point[id_size], c='b', alpha=0.8, edgecolors='none')
                    axs_p[id_subplot//3,id_subplot%3].scatter(-theta_t, r_t, s=size_point[id_size], c='b', alpha=0.8, edgecolors='none')
                if id_quadrant == 1:
                    # negative rho, 2 and 3 quadrant
                    axs_p[id_subplot//3,id_subplot%3].scatter(np.pi - theta_t, r_t, s=size_point[id_size], c='b', alpha=0.8, edgecolors='none')
                    axs_p[id_subplot//3,id_subplot%3].scatter(theta_t - np.pi, r_t, s=size_point[id_size], c='b', alpha=0.8, edgecolors='none')
                ...
                if id_size < num_used_top//2:
                    ...
                    axs_p[id_subplot//3,id_subplot%3].scatter( theta_t_rho, r_t_rho, s=size_point_rho[id_size], c='r', alpha=0.8, edgecolors='none')
                    axs_p[id_subplot//3,id_subplot%3].scatter(-theta_t_rho, r_t_rho, s=size_point_rho[id_size], c='r', alpha=0.8, edgecolors='none')
            else:
                id_pole = id_C
                r_t = Drr_Dict[0,id_pole]
        ...
        axs[id_subplot//3,id_subplot%3].set_title(name_mat.split('.')[0], fontsize=12)
        for t in range(Traj.shape[0]):
            axs[id_subplot//3,id_subplot%3].plot(np.arange(1,37),Traj[t, : , dim_data_draw])
    if draw_weight:
        figs_p.savefig(os.path.join(path_exp, f'weight_poles_{num_used_top}.png'), dpi=400)
    if draw_traj:
...
def main():
    try:
        ...
        path_exp="/home/dan/ws/2021-CrossView/matfiles/1107-dif_act_view"
    ...
    for i in range(5):
        weightPoles(path_exp, theta_max_pi=True, num_used_top=30, dim_data_draw=i, 
                    weight_complex=False, draw_weight=False, draw_traj=False, draw_cmap=True)
