...
class NUCLAskeleton(tudata.Dataset):
    ...
    def __init__(self, root_skeleton="/data/Dan/N-UCLA_MA_3D/skeletons_res",
                root_list="/data/Dan/N-UCLA_MA_3D/lists", phase='train', cam='1', T=0):
        ...
        for view in self.view:
            file_list = os.path.join(self.root_list, f"{view}_{phase}.list")
        ...
        # Print 
        print(f"=== {phase} {self.view}: {len(self.list_samples)} samples ===")
    ...
    def __getitem__(self, index):
        ...
        # global target_view_heat, project_view_heat, heatmap_to_use

        dicts={
            'imgSequence_to_use': imageSequence,
            'sequence_to_use': normSkeleton,
            'heatmap_to_use': heatmap_to_use,
            'actLabel':label_action, 'nframes':self.T, 'info':info, 'view':view
        }
...
if __name__ == "__main__":
    DS = NUCLAskeleton(phase='train',cam='1,2')
